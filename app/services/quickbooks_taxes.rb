    ...
    tax_codes_with_agencies.each do |key, tax_code|
      if is_not_yet_existed(key)
        ...
        if tax_code[:purchase_tax_rate_details].present?
          tax_code[:purchase_tax_rate_details].each do |key, purchase_tax_rate|
            purchase_tax_rate_params = sales_tax_rate_params = tax_code_params(purchase_tax_rate, 'Purchase')
