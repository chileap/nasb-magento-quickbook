module Concerns::MagentoRestApiToken
  ...
  def get_new_access_tokens(authentication_data)
    new_consumer = self.create_consumer(authentication_data)
    new_request_token = self.request_token(consumer: new_consumer, url: authentication_data[:url])
    new_authorize_url = self.get_authorize_url(request_token: new_request_token)
    authorize_new_application = self.authorize_application(authorize_url: new_authorize_url, authentication_data: authentication_data)
    extract_new_oauth_verifier = self.extract_oauth_verifier(callback_page: authorize_new_application, url: authentication_data[:url])
